...
Deep-dive on stream behavior, NDJSON, signal handling, crash-only design, and pager integration for Node.js CLI tools. Buffering and signal details vary by runtime and platform; verify the behavior of the implementation under test. See the main `cli-design` skill for the Unix stream contract, format flags, and exit code conventions. See `output-architecture.md` for TypeScript output ports and formatters. See `testing-cli.md` for testing stream behavior.

---

## 1. Buffering Behavior

Buffered stdout can outperform unbuffered stderr for high-volume piped output because buffering reduces write calls. The ratio is not a portable contract: runtime, platform, stream implementation, payload size, and flush behavior all matter. Benchmark the actual CLI before making performance claims.
...
A traditional C runtime commonly uses `isatty()` to choose a buffering strategy. Node.js streams have their own implementation details, so treat this model as intuition rather than a guarantee:

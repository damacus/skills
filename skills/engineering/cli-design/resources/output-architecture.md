...
TypeScript implementation patterns for building Unix-composable CLI output layers. These examples use a `Result` type and JSON envelope, but neither shape is mandatory. Select and document the project's contract in the main `cli-design` workflow before adapting these examples. See `stream-contracts.md` for buffering, NDJSON, and signal handling. See `testing-cli.md` for testing these patterns.

...
TDD patterns for verifying CLI contracts: stream separation, exit codes, pipe behavior, and output stability. The examples assume JSON output and a particular envelope; test the contract the project actually selected rather than adding those features solely to satisfy these examples. See the main `cli-design` skill for core principles (stream contract, exit codes, format flags). If the project already uses hexagonal architecture, follow its local testing guidance for use cases and test doubles.

---

## 1. Testing stdout/stderr Separation

The most important test for commands that promise pipeable or structured output. A single spinner character on stdout breaks every downstream pipe.
